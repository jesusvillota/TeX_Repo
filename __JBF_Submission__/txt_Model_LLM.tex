...
% << OPTION 1 >>
%The LLM generates both a structured output and accompanying explanations that detail its reasoning process. These explanations serve as a validation mechanism for the structured output.
%By reviewing a random sample of these explanations, we can assess whether the LLM has grasped the task correctly. When the LLM misinterprets instructions, its explanations reveal patterns of confusion or hallucination, signaling that we need to refine the system prompts and function descriptions to provide better guidance. The prompt refinement continues iteratively until the LLM reliably produces correct outputs across different test cases.
...
%To check this, we examine a sample of explanations and see if the LLM correctly uses the schema's parameters and constraints. If we spot any misunderstandings, we improve the prompts and function descriptions. We repeat this process until the LLM reliably produces correct outputs across different test cases.

%The LLM generates both a structured output and accompanying explanations that detail its reasoning process. These explanations serve as a critical validation mechanism, allowing us to verify whether the model correctly interprets the function-calling schema, adheres to the specified parameters and constraints, and follows the system instructions as intended. To assess the LLM's understanding, we review a random sample of these explanations and look for patterns of misinterpretation, confusion, or hallucination. If we identify such issues, we refine the system prompts and function descriptions to provide clearer guidance. This iterative prompt refinement continues until the LLM reliably generates correct outputs across multiple test scenarios
...
%
%Different from KMeans clustering with embeddings, now the distribution profile of articles through clusters becomes very stable across data splits. This evidences that clustering based on a comprehensive analysis of the shocks implied by the article on the affected firms is really stable over time, which constitutes a promising result.
