...
%$\mathcal{Y}_0^i = \langle d_0^i, t_0^i \rangle$, 
...
The two possible cases are illustrated in \cref{fig:trading-timelines}.

%----------------------------------------------------
\input{fig_4__Effective-treatment-date.tex}
%----------------------------------------------------

%For clarity, \cref{fig:trading-timelines} depicts the two scenarios that can arise.
%}


%Since the publication datetime is not necessarily a trading datetime, we cannot directly gauge such an effect by looking at $\mathcal Y_0^i$. 
...
%\bx 
%Throughout our analysis, we will work with daily closing stock market data for each trading day, and we will exploit the time component of $\mathcal Y_0^i$ to assign an \qquote{effective treatment date} to each article. Namely, define $\tilde d_0^i$ as the day at which article $i$'s information can be incorporated into the stock market; then, $\tilde d_0^i$ is the publication date if the article was published on a trading day before the stock market was closed, and is equal to the next closest trading day otherwise. 
%To compute the next closest trading day to $d\in\mathfrak d$ within $\tilde{\mathfrak d}$, 
%%we need to work with a function 
%define $\Lambda:\mathfrak d \to \tilde{\mathfrak d}$ such that  
%$\Lambda(d) 
%:= 
