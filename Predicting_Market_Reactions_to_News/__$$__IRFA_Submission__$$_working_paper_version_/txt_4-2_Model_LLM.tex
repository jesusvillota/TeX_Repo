...
(\cite{le2023bloom}), % Le Scao et al., 2023), 
\texttt{Mixtral} and Meta's \texttt{Llama} series 
(\cite{touvron2023llama}) % Touvron et al., 2023)
emphasizes accessibility and transparency in LLM development.  The latest models, including OpenAI's  \texttt{GPT-4.5}, \texttt{GPT-o1} and \texttt{o3}, Google's \texttt{Gemini 2.5 Pro}, %and \texttt{Mixtral} and \texttt{Gemma},
Anthropic's \texttt{Claude 3.7 Sonnet}, and Meta's \texttt{Llama-3} series
...
In our endeavor we will employ \texttt{Llama-3}, developed by Meta AI and released on April 18, 2024\footnote{
...
\mx 
Each article $i\in\D$ implies a conversation with the LLM. The structure of the conversation consists of defining first a ``system message'', which provides a general context and purpose to the model. In our case:
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{\textwidth}
        ...
        \centering
        \includegraphics[scale=0.48]{fig_7a_LLAMA_Cluster_Distribution.pdf}
        \label{fig:all_data}
    \end{subfigure}
    ...
    \begin{subfigure}[b]{0.32\textwidth}
        ...
        \centering
        \includegraphics[width=\textwidth]{fig_7b_LLAMA_Cluster_Distribution_Train.pdf}
        \label{fig:train_data}
    \end{subfigure}
    \begin{subfigure}[b]{0.32\textwidth}
        \caption{Validation data ($\D^{val}$)}
        \centering
        \includegraphics[width=\textwidth]{fig_7c_LLAMA_Cluster_Distribution_Validation.pdf}
        \label{fig:val_data}
    \end{subfigure}
    \begin{subfigure}[b]{0.32\textwidth}
        \caption{Test data ($\D^{test}$)}
        \centering
        \includegraphics[width=\textwidth]{fig_7d_LLAMA_Cluster_Distribution_Test.pdf}
        \label{fig:test_data}
    \end{subfigure}
